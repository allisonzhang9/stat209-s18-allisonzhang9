...
Answer:
They are correlated and direct (as male mass increases, so does egg mass).
...
ggplot(birds, aes(male_mass,egg_mass))+
  ...
  annotate("text", x = 100, y = 100, label = "Red points = Parrots")
```
...
Answer:
Yes, it shows the direct relationship between the size of the bird and the size of its eggs.
...
ggplot(birds, aes(male_mass,egg_mass))+
  ...
  `scale_y_log10`()+
  geom_smooth(method = "lm", color = "black", se = FALSE, linetype = "dashed", size = 0.5)+
  annotate("text", x = 218, y = 95, label = "Red-tailed Tropicbird")
```
...
```{r}
ggplot(parrots, aes(male_mass, egg_mass))+
  geom_point()+
  ylab("Mass of Eggs")+
  xlab("Mass of Adult Male")+
  ggtitle("Mass of Male vs Mass of Egg for Parrots")
```
