...
**Answer**: 3rd percentile = 369.00, 97th percentile= 4107.46
...
**Answer**: They are pretty similar between both normal and slow speeds. Yes both are significant because they are all positive.
...
**Answer**: Some may take a break while taking the quiz while other people take it very quickly
...
```{r}
model <- lm_basic(dominance ~ 1 + reasoning+ privateness+ baseline, data = pf)
reg_table(model, level = 0.95)
```
...
**Answer**: privateness matches what I expected, but reasoning did not because i expected dominant people to be less reasoning

Take the previous model and add the variables `speed`, `gender`, and
`country`, the latter lumped into 5 categories.

```{r}
model <- lm_basic(dominance ~ 1 + reasoning+ privateness+ baseline +speed+ gender + (fct_lump(country, n=5)), data = pf)
reg_table(model, level = 0.95)
```
...
**Answer**: It is not significant for speed, male is significant (more than female), and significant for CA, IN, and the US (more)
